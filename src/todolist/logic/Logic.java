...
public class Logic {
    ...
    private static final String EXCEPTION_OUTPUT = "Sorry but I don't understand this command!";
    ...
    public void process(String input) {
        ...
        try {
            ...
            uiHandler.sendMessage(EXCEPTION_OUTPUT, true);
            e.printStackTrace();
        }
    }
     ...
     *         task
     */
    public Boolean addRemindBef(String type, String title, String[] arg) {

        String defaultQuantity = "0";

        String defaultTimeUnit = "hour";

        Boolean addResponse = false;

        switch (type) {
        case "event":
            addResponse = addEvent(title, arg[0], arg[1], arg[2], arg[3]);

            if (arg.length < 6) {
                remindBef(title, defaultQuantity, defaultTimeUnit);
            } else {
                remindBef(title, arg[arg.length - 2], arg[arg.length - 1]);
            }
            break;
        case "deadline":
            addResponse = addDeadline(title, arg[0], arg[1]);
